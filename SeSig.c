...
static void _xsig_pipe_handler(
#if NeedFunctionPrototypes
...
XoSignalCallbackProc XoAppAddSignal(context, sig, handler, client_data)
...
XtPointer client_data;        /* private data */
{
  ...
  if (firsttime) {
    firsttime = False;
    pipe(pipefd);
    XtAppAddInput(context, pipefd[0], (XtPointer)XtInputReadMask,
...
void XoAppRemoveSignal(context,
                       sig) XtAppContext context; /* application context */
int sig;                                          /* which signal */
{
...
void XoAppIgnoreSignal(context,
                       sig) XtAppContext context; /* application context */
int sig;                                          /* which signal */
{
 ...
 */
static void _xsig_handler(sig, code) int sig;
int code;
{
  char sig_value;
...
static void _xsig_pipe_handler(client_data, source, id) XtPointer client_data;
int *source;
XtInputId *id;
{
