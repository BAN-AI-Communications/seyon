...
extern void TopAbout(), TopHelp(), TopSet(), TopTransfer(), TopMisc(),
    SetNewlineTrMode(), FunMessage();

extern void upload_acc_ok(), setVal_action_ok(), manual_dial_action_ok(),
    divert_action_ok();

extern Boolean CvtStringToStringArray();

void HangupConfirm(), ExecHangup(), ExitConfirm(), ExitAction(), test();

Boolean inhibit_child = False;
Widget w_exit, w_kill, statusMessage;
...
void InitVariables(topLevel) Widget topLevel;
{
  char buffer[REG_BUF];
...
void SetIcon(topLevelWidget) Widget topLevelWidget;
{
  progIcon = XCreateBitmapFromData(
      XtDisplay(topLevelWidget), DefaultRootWindow(XtDisplay(topLevelWidget)),
      progIcon_bits, progIcon_width, progIcon_height);
  XtVaSetValues(topLevelWidget, XtNiconPixmap, progIcon, NULL);
}

void CreateCommandCenter() {
  void GetQuickKeyResources(), GetValueByPopupOKAction(),
      DispatchActionsCallback();
  void TopShell();
  void KillChildProc();

  Widget mainBox, menuBox, statusBox, messageBox, quickKeyBox;
  ...
  for (i = 0; i < MAX_SEQUICKKEYS; i++) {
    ...
    if (quickKeyRes.visible) {
      n++;
      quickKeyW = AddButton(qKBuf, quickKeyBox, DispatchActionsCallback, NULL);
    }
  }
  if (n == 0)
    XtDestroyWidget(quickKeyBox);
  ...
  if (n)
    XtVaSetValues(quickKeyBox, XtNwidth, menuBoxWidth, NULL);
  ...
  XSetWMProtocols(XtDisplay(topLevel), XtWindow(topLevel), &wm_delete_window,
                  1);
}

void TopAbout(parent) Widget parent;
{
  Widget popup, mBox, uBox, lBox, pic, msg, caption;
  Pixmap pix;
...
#include "authPic.h"
  ...
  pix = XCreateBitmapFromData(XtDisplay(pic), DefaultRootWindow(XtDisplay(pic)),
                              authPic_bits, authPic_width, authPic_height);
...
void TopHelp(widget) Widget widget;
{
...
void DoHangup(widget) Widget widget;
{
  DestroyShell(widget);
  ExecHangup();
}

void HangupConfirm(widget) Widget widget;
{
  Widget popup, dialog;
...
void ExitNoHangup(widget) Widget widget;
{
  DestroyShell(widget);
  s_exit(widget);
}

void ExitHangup(widget) Widget widget;
{
  ExecHangup();
  ExitNoHangup(widget);
}

void ExitConfirm(widget) Widget widget;
{
  Widget popup, dialog;
...
void ExitAction(widget) Widget widget;
{
...
void w_exit_up(w_exit_status) Boolean w_exit_status;
{ XtVaSetValues(w_exit, XtNsensitive, w_exit_status, NULL); }

void SetKillButtonSens(killWidgetStatus) Boolean killWidgetStatus;
{
  XtVaSetValues(w_kill, XtNsensitive, killWidgetStatus, NULL);
  w_exit_up(!killWidgetStatus);
}

void w_kill_up(w_kill_status) Boolean w_kill_status;
{ SetKillButtonSens(w_kill_status); }

void KillChildProc() {
  if (w_child_pid == 0)
    return;
  if (kill(w_child_pid, SIGTERM) == 0)
    w_child_pid = 0;
}

void GetQuickKeyResources(quickKeyName, quickKeyRes) String quickKeyName;
struct _quickKeyRes *quickKeyRes;
{
#define offset(field) XtOffsetOf(struct _quickKeyRes, field)
  static XtResource resources[] = {
      {"visible", "Visible", XtRBoolean, sizeof(Boolean), offset(visible),
       XtRImmediate, (XtPointer)False},
      {"action", "Action", XtRString, sizeof(String), offset(action), XtRString,
       (XtPointer) ""},
  };
#undef offset
...
#define offset(field) XtOffsetOf(struct QueryResources, field)
  ...
  static XtResource resources[] = {
      {"modems", "Modems", XtRString, sizeof(String), offset(modems), XtRString,
       (XtPointer) ""},
      {"script", "Script", XtRString, sizeof(String), offset(script), XtRString,
       (XtPointer)NULL},
      ...
      {"backspaceTranslation", "BackspaceTranslation", XtRBoolean,
       ...
       (XtPointer)False},
      {"metaKeyTranslation", "MetaKeyTranslation", XtRBoolean, sizeof(Boolean),
       offset(metaKeyTranslation), XtRImmediate, (XtPointer)True},
      {"xonxoffFlowControl", "XonxoffFlowControl", XtRBoolean, sizeof(Boolean),
       offset(xonxoffFlowControl), XtRImmediate, (XtPointer)False},
      {"rtsctsFlowControl", "RtsctsFlowControl", XtRBoolean, sizeof(Boolean),
       offset(rtsctsFlowControl), XtRImmediate, (XtPointer)False},
      {"newlineTranslation", "NewlineTranslation", XtRString, sizeof(String),
       offset(newlineTranslation), XtRImmediate, (XtPointer) "cr"},
      ...
      {"dialTimeOut", "DialTimeOut", XtRInt, sizeof(int), offset(dialTimeOut),
       XtRImmediate, (XtPointer)45},
      {"dialDelay", "DialDelay", XtRInt, sizeof(int), offset(dialDelay),
       XtRImmediate, (XtPointer)10},
      {"dialRepeat", "DialRepeat", XtRInt, sizeof(int), offset(dialRepeat),
      ...
      {"startupAction", "StartupAction", XtRString, sizeof(String),
       offset(startupAction), XtRString, (XtPointer) "RunScript(startup);"},
      ...
      {"autoZmodemAction", "AutoZmodemAction", XtRString, sizeof(String),
       offset(autoZmodemAction), XtRString, (XtPointer) "ShellCommand($rz);"},

      {"modemVMin", "ModemVMin", XtRInt, sizeof(int), offset(modemVMin),
      ...
      {"modemAttentionString", "ModemAttentionString", XtRString,
       sizeof(String), offset(modemAttentionString), XtRString,
       (XtPointer) "+++"},
      ...
      {"neverBeep", "NeverBeep", XtRBoolean, sizeof(Boolean), offset(neverBeep),
       XtRImmediate, (XtPointer)False},
      ...
      {"phoneFile", "PhoneFile", XtRString, sizeof(String), offset(phoneFile),
       XtRString, (XtPointer) "phonelist"},
      ...
      {"helpFile", "HelpFile", XtRString, sizeof(String), offset(helpFile),
       XtRString, (XtPointer)HELPFILE},

      {"modemStatusInterval", "ModemStatusInterval", XtRInt, sizeof(int),
       offset(modemStatusInterval), XtRImmediate, (XtPointer)5},

      {"idleGuard", "IdleGuard", XtRBoolean, sizeof(Boolean), offset(idleGuard),
       XtRImmediate, (XtPointer)False},
...
#undef offset
  ...
  XtSetTypeConverter(XtRString, XtRStringArray, CvtStringToStringArray, NULL, 0,
                     XtCacheNone, NULL);
