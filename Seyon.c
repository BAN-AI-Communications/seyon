...
#if HAVE_TERMIOS
#include <termios.h>
struct termios newmode, oldmode;
#else
#if HAVE_TERMIO
#include <termio.h>
...
extern void SetIcon(), GetResources(), InitVariables();
extern void PopupInitError(), PopupFatalError();

void s_script(), s_set(), s_exit(), die(), cleanup_exit();

int tfd, child_pipe[2], scriptToMainPipe[2], mainToTermPipe[2];
FILE *tfp;
...
int main(int argc, char *argv[]) {
  int OpenModem();
  void DispatchActions(), GetParameters(), ShowOpenModemErrMsg();
  ...
  char modemList[LRG_BUF], *curModem, *startupAction = modemList;
...
#include "Seyon.ad.h"
  ...
  for (i = 1;
       i < sepIndex && strncmp(argv[i], "-noemulator", max(4, strlen(argv[i])));
       i++)
    ;
  ...
  if (i >= sepIndex) {
    ...
    for (i = 1;
         i < sepIndex && strncmp(argv[i], "-emulator", max(3, strlen(argv[i])));
         i++)
      ;
    if (i < sepIndex - 1)
    ...
    for (i = 1; i < sepIndex &&
                strncmp(argv[i], "-nodefargs", max(5, strlen(argv[i])));
         i++)
      ;
    ...
    fprintf(stderr, "%s `%s.\n%s\n", ">> Warning: Could not execute", termEmu,
  ...
  topLevel =
      XtAppInitialize(&appContext, "Seyon", optionList, XtNumber(optionList),
  ...
  do {
    if ((retStatus = OpenModem(curModem)) >= 0)
      break;
  ...
  if (qres.showFunMessages)
    XtAppAddTimeOut(appContext, qres.funMessagesInterval * 1000, FunMessage,
                    NULL);
  ...
  if (qres.script) {
    ...
    if (length_remaining > length) {
      sprintf(startupAction + strlen(startupAction), " RunScript(%s);",
              qres.script);
      length_remaining -= length;
    } else {
      printf("String overflow in parsing options\n");
