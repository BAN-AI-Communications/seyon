...
#define FontMaxCharW(f)                                                        \
  ((f)->max_bounds.rbearing - (f)->min_bounds.lbearing + 1)
...
static void Redisplay(XfwfMultiListWidget mlw, XEvent *event,
...
static Boolean SetValues(XfwfMultiListWidget cpl, XfwfMultiListWidget rpl,
...
static Boolean Layout(XfwfMultiListWidget mlw, Boolean w_changeable,
                      Boolean h_changeable, Dimension *w_ptr, Dimension *h_ptr);
...
static void PixelToRowColumn(XfwfMultiListWidget mlw, int x, int y,
                             int *row_ptr, int *column_ptr);
static void RowColumnToPixels(XfwfMultiListWidget mlw, int row, int col,
                              int *x_ptr, int *y_ptr, int *w_ptr, int *h_ptr);
static Boolean RowColumnToItem(XfwfMultiListWidget mlw, int row, int column,
                               int *item_ptr);
static Boolean ItemToRowColumn(XfwfMultiListWidget mlw, int item_index,
                               int *row_ptr, int *column_ptr);
static void Select(XfwfMultiListWidget mlw, XEvent *event, String *params,
                   Cardinal *num_params);
static void Unselect(XfwfMultiListWidget mlw, XEvent *event, String *params,
                     Cardinal *num_params);
static void Toggle(XfwfMultiListWidget mlw, XEvent *event, String *params,
                   Cardinal *num_params);
static void Extend(XfwfMultiListWidget mlw, XEvent *event, String *params,
                   Cardinal *num_params);
static void Notify(XfwfMultiListWidget mlw, XEvent *event, String *params,
                   Cardinal *num_params);
#endif
...
static XtResource resources[] = {
    ...
    {XtNcursor, XtCCursor, XtRCursor, sizeof(Cursor), SimpleFieldOffset(cursor),
     XtRString, "left_ptr"},
    ...
    {XtNlongest, XtCLongest, XtRInt, sizeof(int), MultiListFieldOffset(longest),
    ...
    {XtNlist, XtCList, XtRPointer, sizeof(char **), MultiListFieldOffset(list),
     XtRString, NULL},
...
static XtActionsRec actions[] = {
    {"Select", (XtActionProc)Select}, {"Unselect", (XtActionProc)Unselect},
    {"Toggle", (XtActionProc)Toggle}, {"Extend", (XtActionProc)Extend},
    {"Notify", (XtActionProc)Notify}, {NULL, (XtActionProc)NULL}};
...
static void RecalcCoords(XfwfMultiListWidget mlw, Boolean width_changeable,
                         Boolean height_changeable)
#else
static void RecalcCoords(mlw, width_changeable,
                         height_changeable) XfwfMultiListWidget mlw;
...
static void NegotiateSizeChange(XfwfMultiListWidget mlw, Dimension width,
                                Dimension height)
#else
static void NegotiateSizeChange(mlw, width, height) XfwfMultiListWidget mlw;
Dimension width, height;
#endif
{
  int attempt_number;
...
static Boolean Layout(XfwfMultiListWidget mlw, Boolean w_changeable,
                      Boolean h_changeable, Dimension *w_ptr, Dimension *h_ptr)
#else
static Boolean Layout(mlw, w_changeable, h_changeable, w_ptr,
...
int row, column;
{
  ...
  if (has_item == True) {
    text_h = min(FontH(MultiListFont(mlw)) + (int)MultiListRowSpace(mlw),
    ...
    XDrawString(XtDisplay(mlw), XtWindow(mlw), fg_gc, str_x, str_y,
                MultiListItemString(item), strlen(MultiListItemString(item)));
  }
} /* End RedrawRowColumn */
...
Cardinal *num_params;
{
  ...
  if (ret_value.item == -1)
    ...
    ret_value.string =
        MultiListItemString(MultiListNthItem(mlw, ret_value.item));
...
void XfwfMultiListSetNewData(XfwfMultiListWidget mlw, String *list, int nitems,
                             int longest, int resize,
                             Boolean *sensitivity_array)
#else
void XfwfMultiListSetNewData(mlw, list, nitems, longest, resize,
                             sensitivity_array) XfwfMultiListWidget mlw;
String *list;
int nitems, longest;
int resize;
Boolean *sensitivity_array;
